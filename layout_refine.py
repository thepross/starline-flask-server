...
def get_refine_bboxes(initial_data, iteration_rounds = 10):
    ...
    shifted_bbox = shifted_bbox.float()
    
    bbox_distrib_map = torch.tensor(bbox_distrib_map).float().unsqueeze(0).unsqueeze(0)
    bbox_mask = torch.tensor(bbox_mask).float()
    smooth_region_mask = torch.tensor(smooth_region_mask).float()
    len_info = len_info
    ...
    for _ in range(iteration_rounds):
        ...
        shifted_bbox = shifted_bbox.cpu()
        shifted_mask = torch.tensor(next_mask).cpu()
